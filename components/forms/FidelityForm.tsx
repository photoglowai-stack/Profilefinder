    ...
    <div className="flex flex-col gap-4">
      ...
      {previews.length === 0 ? (
        <label
          ...
          className="group border-2 border-dashed border-slate-300 rounded-2xl p-5 flex flex-col items-center justify-center cursor-pointer hover:border-[#ff4e71] hover:bg-rose-50/50 transition-all"
        >
          <div className="p-2.5 bg-gradient-to-br from-[#ff4e71] to-[#ff7f66] rounded-xl mb-2 group-hover:scale-110 transition-transform">
        ...
        <div>
          ...
          <label
            htmlFor="screenshots-upload"
            className="block text-center text-[11px] text-[#ff4e71] cursor-pointer font-semibold"
          >
            + Add more
          </label>
          <input
      ...
      <div className="flex flex-wrap gap-1.5">
        {features.slice(0, 4).map((item, index) => (
          <span key={index} className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-600 bg-slate-50 border border-slate-100 rounded-full px-2.5 py-1">
            <span className="w-1.5 h-1.5 bg-[#ff4e71] rounded-full shrink-0" />
            {item}
          </span>
        ))}
      </div>
      ...
      <motion.button
        ...
        className={`w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-full font-bold text-white transition-all ${screenshots.length > 0 && !isSearching
            ? "bg-gradient-to-r from-[#ff4e71] to-[#ff7f66] shadow-[0_8px_24px_rgba(255,78,113,0.3)] hover:shadow-lg cursor-pointer"
