...
export default function ActivityTracker() {
    ...
    // Target from Context or Storage
    const [targetName, setTargetName] = useState("Target");

    useEffect(() => {
        if (typeof window !== 'undefined') {
            const stored = sessionStorage.getItem('pf_following_target') || "@instagram_handle";
            setTargetName(stored);
        }
        startLoading();
    }, []);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            router.push('/payment/instagram');
        }, 1000);
    };

    return (
        <ServiceLayout variant="activity">
            <div className="dating-search-wrapper">
                <div className="dating-card">
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <span style={{ fontSize: '12px', fontWeight: 500, color: '#6b7280' }}>
                                {currentStep === 1 ? 'Fetching..' : 'Complete'}
                            </span>
                            <div style={{ position: 'relative' }}>
                                <img
                                    src={'https://ui-avatars.com/api/?name=Ig&background=a855f7&color=fff'}
                                    alt="Target"
                                    style={{ width: '36px', height: '36px', borderRadius: '50%', border: '2px solid #a855f7', padding: '2px', objectFit: 'cover' }}
                                />
                                {currentStep === 2 && (
                                    <div style={{
                                        position: 'absolute',
                    ...
                    <div className={`dating-step ${currentStep === 2 ? 'active' : ''}`} style={{ textAlign: 'center', alignItems: 'center' }}>
                        ...
                        <div className="dating-check-circle" style={{ marginBottom: '16px', backgroundColor: '#f3e8ff' }}>
                            <div style={{
                                width: '40px',
                                height: '40px',
                                backgroundColor: '#a855f7',
                        ...
                        <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#111', marginBottom: '8px' }}>Ready!</h2>
                        <p style={{ fontSize: '14px', color: '#6b7280', marginBottom: '32px', maxWidth: '280px', lineHeight: 1.6 }}>
                            Scan complete. We detected <span style={{ backgroundColor: '#f3e8ff', color: '#7e22ce', fontWeight: 700, padding: '2px 8px', borderRadius: '4px' }}>3 suspicious profiles</span> interacting recently.
                        </p>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'center', gap: '24px', marginTop: '32px', fontSize: '12px', fontWeight: 600, color: '#9ca3af' }}>
                            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <Lock size={14} style={{ color: '#a855f7' }} /> Secure Data
                            </span>
                            <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
